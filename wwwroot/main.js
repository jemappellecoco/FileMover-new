...
const pages = {
  ...
  // ftp:     document.getElementById('page-ftp'),  
};
...
const roots = {
  ...
  archive: document.getElementById('archive-root'),
  nodes:   document.getElementById('nodes-root'), 
  // ftp:     document.getElementById('ftp-root'),
};
...
tabButtons.forEach(btn => {
  btn.addEventListener('click', async () => {
    ...
    Object.keys(pages).forEach(k => pages[k].classList.toggle('active', k === tab));
    ...
    if (tab === 'archive') {
      if (!inited.archive) {
        const { initArchive } = await import('./archive.js');
        initArchive(roots.archive);
        inited.archive = true;
      } else {
        window.dispatchEvent(new CustomEvent('archive-reload'));
      }
      return;
    }
